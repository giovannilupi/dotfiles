...
ffile() {
    local -a fd_opts=("--color=always" "--type=file" "--follow")
    local -a args=()
...
fdir() {
    local -a fd_opts=("--color=always" "--type=directory" "--follow")
    local -a args=()
    local use_glob_strings=true
    # Iterate through arguments and build command
    while [[ $# -gt 0 ]]; do
...
fline() {
    local -a rg_opts=("--color=always" "--line-number" "--no-heading" "--follow")
    local -a args=()
